...
from utils.ETF_utils import get_global_etf, update_global_etf, update_modality_etf, update_modality_etf_norm
...
def cos_similarity_matrix(x1, x2, temp=None):
    s_matrix = torch.zeros(x1.shape[0], x2.shape[0]).to(x1.device)
    for i in range(x1.shape[0]):
        for j in range(x2.shape[0]):
            s = F.cosine_similarity(x1[i], x2[j], dim=0) * temp
            s_matrix[i, j] = s
    return s_matrix
    ...
    for step, (spec, image, label) in enumerate(dataloader):
        ...
        if warmup:
            ...
            if args.modulation_starts <= epoch < args.modulation_ends:
                ...
                ######################### similarity loss #################################
                # pass
                audio_sim = cos_similarity_matrix(a, ETF_proto_a, temp=args.temperature)  # 30
                loss_proto_a = criterion(audio_sim, label)
                visual_sim = cos_similarity_matrix(v, ETF_proto_v, temp=args.temperature)
...
def main():
    ...
    if args.train:
        ...
        trainloss_file = args.logs_path + '/ETF-norm/' + args.dataset + '/train_log-alpha{}-lr{}-fusion_{}-CI_{}-optim_{}-Mom_{}-AP{}-T{}-coef1-{}.txt'.\
            format(args.alpha, args.learning_rate, args.fusion_method, str(args.class_imbalance), args.optimizer, args.momentum_coef, args.adaptive_epoch, args.temperature, coef1)
        if not os.path.exists(args.logs_path + '/ETF-norm/' + args.dataset):
            os.makedirs(args.logs_path + '/ETF-norm/' + args.dataset)

        save_path = args.ckpt_path + '/ETF-norm' + '/model-' + args.dataset + '/train_log-alpha{}-lr{}-fusion_{}-CI_{}-optim_{}-Mom_{}-AP{}-T{}-coef1_{}'.\
            format(args.alpha, args.learning_rate, args.fusion_method, str(args.class_imbalance), args.optimizer, args.momentum_coef, args.adaptive_epoch, args.temperature, coef1)
        ...
        for epoch in range(args.epochs):
            ...
            ETF_proto_a, ETF_proto_v, audio_proto, visual_proto = update_modality_etf_norm(args, ETF_proto_a, ETF_proto_v, train_dataloader, model, device, ratio=1.0)
