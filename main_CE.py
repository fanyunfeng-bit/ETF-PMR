...
def train_epoch(args, epoch, model, device, dataloader, optimizer, scheduler, writer=None):
    ...
    angle_file = args.logs_path + '/Method-CE' + '/angle-' + args.dataset + '-' + args.fusion_method + '-bsz' + \
                 str(args.batch_size) + '-lr' + str(args.learning_rate) \
                 + '-epoch' + str(args.epochs) + '-' + args.modulation + str(args.alpha) + '.txt'
...
def main():
    ...
    if args.train:
        ...
        trainloss_file = args.logs_path + '/Method-CE' + '/train_loss-' + args.dataset + '-' + args.fusion_method + '-bsz' + \
                         str(args.batch_size) + '-lr' + str(args.learning_rate) \
                         + '-epoch' + str(args.epochs) + '-' + args.modulation + str(args.alpha) + '.txt'
        if not os.path.exists(args.logs_path + '/Method-CE'):
            os.makedirs(args.logs_path + '/Method-CE')

        save_path = args.ckpt_path + '/Method-CE' + '/model-' + args.dataset + '-' + args.fusion_method + '-bsz' + \
                    str(args.batch_size) + '-lr' + str(args.learning_rate) \
        ...
        for epoch in range(args.epochs):
            ...
            s_time = time.time()
            batch_loss, batch_loss_a, batch_loss_v, a_angle, v_angle = train_epoch(args, epoch, model, device,
