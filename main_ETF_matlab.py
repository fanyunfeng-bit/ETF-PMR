...
def main():
    ...
    if args.train:
        ...
        trainloss_file = args.logs_path + '/ETF-matlab/' + args.dataset + '/dist-optim_{}'.format(
        ...
        if not os.path.exists(
                args.logs_path + '/ETF-matlab/' + args.dataset + '/dist-optim_{}'.format(str(args.distance_optim))):
            os.makedirs(
                args.logs_path + '/ETF-matlab/' + args.dataset + '/dist-optim_{}'.format(str(args.distance_optim)))

        save_path = args.ckpt_path + '/ETF-matlab' + '/model-' + args.dataset + '/dist-optim_{}'.format(
        ...
        # print(ETF_proto.shape, ETF_proto[2, :])
        # center = torch.zeros_like(ETF_proto[0, :]).to(device)
        # for i in range(0, 28):
        # #     center += ETF_proto[i, :]
        # # print(center / 28)
        #     for j in range(i+1, 28):
        #         print(torch.nn.functional.cosine_similarity(ETF_proto[i, :], ETF_proto[j, :], dim=0))
        ...
        for epoch in range(args.epochs):
            ...
            if epoch >= args.adaptive_epoch and epoch % args.proto_update_freq == 0:
                # ETF_proto_a, ETF_proto_v = update_modality_etf(args, ETF_proto_a, ETF_proto_v, train_dataloader, model,
                #                                                device, ratio=1.0)
