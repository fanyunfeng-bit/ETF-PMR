...
from utils.ETF_utils import get_global_etf, update_global_etf, update_modality_etf, save_separate_feature
from utils.utils import setup_seed, weight_init, calculate_prototype
...
def get_arguments():
    ...
    parser.add_argument('--momentum_coef', default=0.95, type=float)  # ETF
    ...
    parser.add_argument('--adaptive_epoch', default=10, type=int, help='')

    parser.add_argument('--class_imbalance', action='store_true')
    parser.add_argument('--distance_optim', action='store_true')
...
def train_epoch(args, epoch, model, device, dataloader, optimizer, scheduler, ETF_proto_a, ETF_proto_v, audio_proto,
                visual_proto, warmup=True):
    ...
    for step, (spec, image, label) in enumerate(dataloader):
        ...
        if warmup:
            ...
            if args.modulation_starts <= epoch < args.modulation_ends:
                ...
                # we find that it is simple and effective with a fixed coef1=0.9
                if ratio_a_p > 1:
                    beta = coef1 * args.alpha  # audio coef
                    lam = 1 * args.alpha  # visual coef
                elif ratio_a_p < 1:
                    beta = 1 * args.alpha
                    lam = coef1 * args.alpha
                else:
                    beta = coef1 * args.alpha
                    lam = coef1 * args.alpha

                # # sigmoid manner
                # if ratio_a_p > 1:
                #     beta = (1.5-F.sigmoid(ratio_a_p-1)) * args.alpha  # audio coef
                #     lam = 1 * args.alpha  # visual coef
                # elif ratio_a_p < 1:
                #     beta = 1 * args.alpha
                #     lam = (1.5-F.sigmoid(1 - ratio_a_p)) * args.alpha
                # else:
                #     beta = 1.0 * args.alpha
                #     lam = 1.0 * args.alpha

                if args.distance_optim:
                    distances_a = 0
                    distances_v = 0
                    for i in range(label.shape[0]):
                        ######################### distance loss #################################
                        h_etf_a = ETF_proto_a[label[i], :]
                        h_etf_v = ETF_proto_v[label[i], :]
                        distance_a = (torch.dist(a[i], h_etf_a, p=2)) ** 2 / label.shape[0]
                        distance_v = (torch.dist(v[i], h_etf_v, p=2)) ** 2 / label.shape[0]
                        assert (torch.isnan(distance_a).sum() == 0)
                        assert (torch.isnan(distance_v).sum() == 0)
                        distances_a += distance_a
                        distances_v += distance_v
                    loss_etf = beta * distances_a + lam * distances_v
                else:
                    ######################### similarity loss #################################
                    audio_sim = -EU_dist(a, ETF_proto_a)  # B x n_class
                    visual_sim = -EU_dist(v, ETF_proto_v)  # B x n_class
                    loss_proto_a = criterion(audio_sim, label)
                    loss_proto_v = criterion(visual_sim, label)
                    loss_etf = beta * loss_proto_a + lam * loss_proto_v
                    ##########################################################################
...
def main():
    ...
    if args.train:
        ...
        trainloss_file = args.logs_path + '/ETF-matlab-init-updatefrequency/' + args.dataset + '/dist-optim_{}'.format(
            str(args.distance_optim)) + '/train_log-alpha{}-lr{}-fusion_{}-CI_{}-optim_{}-Mom_{}-AP{}-freq{}-coef1-{}.txt'.format(
            args.alpha, args.learning_rate, args.fusion_method, str(args.class_imbalance), args.optimizer,
            args.momentum_coef, args.adaptive_epoch, args.proto_update_freq, coef1)
        if not os.path.exists(
                args.logs_path + '/ETF-matlab-init-updatefrequency/' + args.dataset + '/dist-optim_{}'.format(str(args.distance_optim))):
            os.makedirs(
                args.logs_path + '/ETF-matlab-init-updatefrequency/' + args.dataset + '/dist-optim_{}'.format(str(args.distance_optim)))

        save_path = args.ckpt_path + '/ETF-matlab-init-updatefrequency' + '/model-' + args.dataset + '/dist-optim_{}'.format(
            str(args.distance_optim)) + '/train_log-alpha{}-lr{}-fusion_{}-CI_{}-optim_{}-Mom_{}-AP{}-freq{}-coef1-{}'.format(
            args.alpha, args.learning_rate, args.fusion_method, str(args.class_imbalance), args.optimizer,
            args.momentum_coef, args.adaptive_epoch, args.proto_update_freq, coef1)
        ...
        # matlab
        a_feature_file = "{}-Imbala_{}-feature_a_matrix.mat".format(args.dataset, str(args.class_imbalance))
        a_ETF_file = "{}-Imbala_{}-ETF_a_matrix.mat".format(args.dataset, str(args.class_imbalance))
        v_feature_file = "{}-Imbala_{}-feature_v_matrix.mat".format(args.dataset, str(args.class_imbalance))
        v_ETF_file = "{}-Imbala_{}-ETF_v_matrix.mat".format(args.dataset, str(args.class_imbalance))
        mm_feature_file = "{}-Imbala_{}-feature_mm_matrix.mat".format(args.dataset, str(args.class_imbalance))
        mm_ETF_file = "{}-Imbala_{}-ETF_mm_matrix.mat".format(args.dataset, str(args.class_imbalance))

        _, _ = save_separate_feature(args, train_dataloader, model, device, a_feature_file,
                                     v_feature_file, mm_feature_file)

        eng.optimalETF(a_feature_file, a_ETF_file, args.dataset)
        eng.optimalETF(v_feature_file, v_ETF_file, args.dataset)
        eng.optimalETF(mm_feature_file, mm_ETF_file, args.dataset)

        a_etf = scio.loadmat('OptM-master/' + a_ETF_file)['W_p']
        ETF_proto_a = torch.tensor(a_etf).T.to(device)
        v_etf = scio.loadmat('OptM-master/' + v_ETF_file)['W_p']
        ETF_proto_v = torch.tensor(v_etf).T.to(device)
        mm_etf = scio.loadmat('OptM-master/' + mm_ETF_file)['W_p']
        ETF_proto_mm = torch.tensor(mm_etf).T.to(device)


        ETF_proto_a = args.momentum_coef * ETF_proto_mm + (1 - args.momentum_coef) * ETF_proto_a
        ETF_proto_v = args.momentum_coef * ETF_proto_mm + (1 - args.momentum_coef) * ETF_proto_v

        # ETF_proto_a = args.momentum_coef * ETF_proto_a + (1 - args.momentum_coef) * ETF_proto_a
        # ETF_proto_v = args.momentum_coef * ETF_proto_v + (1 - args.momentum_coef) * ETF_proto_v

        # feature_size = args.embed_dim
        # ETF_proto = get_global_etf(feature_size, n_classes, pre_generated=False).to(device)  # bsz x embed_dim 模长为1,中心点为0
        # ETF_proto = update_global_etf(args, ETF_proto, train_dataloader, model, device)
        # ETF_proto_a = ETF_proto_v = ETF_proto

        audio_proto, visual_proto = calculate_prototype(args, model, train_dataloader, device, ratio=0.5)
        for epoch in range(args.epochs):
            ...
            if epoch <= args.warmup_epoch:
                batch_loss, batch_loss_etf = train_epoch(args, epoch, model, device, train_dataloader, optimizer,
                                                         scheduler, ETF_proto_a, ETF_proto_v, audio_proto, visual_proto,
                                                         warmup=True)
            else:
                batch_loss, batch_loss_etf = train_epoch(args, epoch, model, device, train_dataloader, optimizer,
                                                         scheduler, ETF_proto_a, ETF_proto_v, audio_proto, visual_proto,
            ...
            #     ETF_proto_update = update_global_etf(args, ETF_proto, train_dataloader, model, device)
            #     ETF_proto_a = ETF_proto_v = ETF_proto_update

            if epoch >= args.adaptive_epoch and epoch % args.proto_update_freq == 0:
                ETF_proto_a, ETF_proto_v, audio_proto, visual_proto = update_modality_etf(args, ETF_proto_a, ETF_proto_v, train_dataloader, model,
                                                               device, ratio=1.0)

                # audio_proto, visual_proto = save_separate_feature(args, train_dataloader, model, device, a_feature_file,
                #                                                   v_feature_file, mm_feature_file)
                ...
                # # eng.optimalETF(mm_feature_file, mm_ETF_file, args.dataset)
                # a_etf = scio.loadmat('OptM-master/' + a_ETF_file)['W_p']
                # ETF_proto_a_ = torch.tensor(a_etf).T.to(device)
                # v_etf = scio.loadmat('OptM-master/' + v_ETF_file)['W_p']
                # ETF_proto_v_ = torch.tensor(v_etf).T.to(device)
                # # mm_etf = scio.loadmat('OptM-master/' + mm_ETF_file)['W_p']
                # # ETF_proto_mm = torch.tensor(mm_etf).T.to(device)
                # ETF_proto_a = args.momentum_coef * ETF_proto_a + (1 - args.momentum_coef) * ETF_proto_a_
                # ETF_proto_v = args.momentum_coef * ETF_proto_v + (1 - args.momentum_coef) * ETF_proto_v_
            ...
            # if args.dataset == 'AVE':
            #     audio_proto, visual_proto = calculate_prototype(args, model, val_dataloader, device, ratio=1.0)
            # elif args.dataset == 'CGMNIST':
            #     audio_proto, visual_proto = calculate_prototype(args, model, val_dataloader, device, ratio=1.0)
            # else:
            #     audio_proto, visual_proto = calculate_prototype(args, model, train_dataloader, device, ratio=0.5)
            ...
            if acc > best_acc or (epoch + 1) % 10 == 0:
                ...
                torch.save(
                    ...
                    os.path.join(save_path, 'save_model.pt'.format(epoch))
                )
                print('Saved model!!!')
        f_trainloss.close()
