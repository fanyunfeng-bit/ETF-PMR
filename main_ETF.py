...
from utils.ETF_utils import get_global_etf, update_global_etf, update_modality_etf
...
def train_epoch(args, epoch, model, device, dataloader, optimizer, scheduler, ETF_proto_a, ETF_proto_v, audio_proto, visual_proto, warmup=True):
...
def main():
    ...
    if args.train:
        ...
        trainloss_file = args.logs_path + '/ETF/' + args.dataset + '/dist-optim_{}'.format(str(args.distance_optim)) + '/train_log-alpha{}-lr{}-fusion_{}-CI_{}-optim_{}-Mom_{}-AP{}-coef1_{}.txt'.format(args.alpha, args.learning_rate, args.fusion_method, str(args.class_imbalance), args.optimizer, args.momentum_coef, args.adaptive_epoch, coef1)
        if not os.path.exists(args.logs_path + '/ETF/' + args.dataset + '/dist-optim_{}'.format(str(args.distance_optim))):
            os.makedirs(args.logs_path + '/ETF/' + args.dataset + '/dist-optim_{}'.format(str(args.distance_optim)))

        save_path = args.ckpt_path + '/ETF' + '/model-' + args.dataset + '/dist-optim_{}'.format(str(args.distance_optim)) + '/train_log-alpha{}-lr{}-fusion_{}-CI_{}-optim_{}-Mom_{}-AP{}-coef1_{}'.format(args.alpha, args.learning_rate, args.fusion_method, str(args.class_imbalance), args.optimizer, args.momentum_coef, args.adaptive_epoch, coef1)
        ...
        for epoch in range(args.epochs):
            ...
            if epoch <= args.warmup_epoch:
                batch_loss, batch_loss_etf = train_epoch(args, epoch, model, device, train_dataloader, optimizer,
                                                         scheduler, ETF_proto_a, ETF_proto_v, audio_proto, visual_proto, warmup=True)
            else:
                batch_loss, batch_loss_etf = train_epoch(args, epoch, model, device, train_dataloader, optimizer,
                                                         scheduler, ETF_proto_a, ETF_proto_v, audio_proto, visual_proto, warmup=False)
            e_time = time.time()
            ...
            if epoch >= args.adaptive_epoch and epoch % args.proto_update_freq == 0:
                ETF_proto_a, ETF_proto_v = update_modality_etf(args, ETF_proto_a, ETF_proto_v, train_dataloader, model, device, ratio=1.0)
